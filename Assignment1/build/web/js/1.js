document.addEventListener("DOMContentLoaded", function () {
    ...
    var vuong = document.querySelector('.vuong');
    var login_success = document.querySelector('.login_success');
    console.log(user)
    console.log("h:" + login.getAttribute('data-hienlen'));

    if (login.getAttribute('data-hienlen') != 'hienlen') {
        ...
        user.addEventListener('click', function () {
            vuong.classList.toggle('turn-off');
            login_success.classList.toggle('turn-off');

        })
    }
    //Xử lí button phân trang            
    var nut_phantrang = document.querySelectorAll('ul.pagination li a');
  
    for (var i = 0; i < nut_phantrang.length; i++) {

        var chon = nut_phantrang[i].getAttribute('data-size_pt');
        if (chon != null) {
            nut_phantrang[i].classList.add('sd_pt');
        }
        nut_phantrang[i].onclick = function () {

            for (var k = 0; k < nut_phantrang.length; k++) {
                nut_phantrang[k].classList.remove('sd_pt');
            }
            this.classList.add('sd_pt');
